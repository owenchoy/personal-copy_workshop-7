...
The 'abalone_growth.csv' file is in the 'data' file in my git repository (on my laptop).

Note the file path within the `read_csv()` function. How is this different from how we've stored and read data before?  

This is different from before because it is using the 'here' function and saying 'data' is the file to pull 'abalone_growth.csv'.
...
'str' shows the number of data values in each column, some of the data, and the column headers. 
'glimpse' shows how many columns there are, the column headers, and more values.
'class' shows some random stuff idk at all. 
